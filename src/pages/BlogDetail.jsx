...
const BlogDetail = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [blogData, recentData] = await Promise.all([
        blogQueries.getBlogBySlug(slug),
        blogQueries.getRecentPosts(slug, 5)
      ]);

      setBlog(blogData);
      setRecentPosts(recentData);

      // Fetch related posts after we have the blog data
      if (blogData && blogData.category) {
        const relatedData = await blogQueries.getRelatedPosts(blogData.category, slug, 3);
  ...
  const createCta = (title, description, buttonText, link) => {
    return `
