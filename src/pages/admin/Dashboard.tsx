...
import { Users, MessageSquare, LogOut, BookOpen, Menu, ChevronLeft, ChevronRight } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      {/* Sidebar */}
      <aside className={`hidden md:flex flex-col ${sidebarOpen ? 'w-64' : 'w-20'} bg-white border-r shadow-sm min-h-screen transition-all duration-300 relative`}>
        {/* Always show the toggle button at the top */}
        <div className="flex items-center h-16 border-b px-2 relative">
          {sidebarOpen && (
            <span className="text-xl font-bold text-brand-green transition-opacity duration-200">Admin Panel</span>
          )}
          <button
        ...
        <nav className="flex-1 py-6 px-2 space-y-2">
          ...
          >
            ...
            <span className={`transition-all duration-200 ${sidebarOpen ? 'inline' : 'hidden'}`}>Appointments</span>
          </button>
          <button
          ...
          >
            ...
            <span className={`transition-all duration-200 ${sidebarOpen ? 'inline' : 'hidden'}`}>Contact Messages</span>
          </button>
          <button
          ...
          >
            ...
            <span className={`transition-all duration-200 ${sidebarOpen ? 'inline' : 'hidden'}`}>Blog Management</span>
          </button>
        </nav>
        <div className="mt-auto p-4 border-t">
          <div className={`flex items-center gap-2 justify-${sidebarOpen ? 'between' : 'center'}`}>
            {sidebarOpen && <span className="text-xs text-gray-500 truncate">{user?.email}</span>}
            <Button 
              variant="outline" 
              size="sm" 
