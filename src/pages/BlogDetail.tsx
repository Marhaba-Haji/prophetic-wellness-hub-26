...
const BlogDetail = () => {
  ...
  if (loading) {
    return (
      <Layout>
        <Helmet>
          <title>Loading... - RevivoHeal Blog</title>
  ...
  if (!blog) {
    return (
      <Layout>
        <Helmet>
          <title>Blog Post Not Found - RevivoHeal</title>
          <meta name="description" content="The requested blog post could not be found on RevivoHeal." />
          <meta name="robots" content="noindex, nofollow" />
        </Helmet>
        <div className="container mx-auto px-4 py-8">
  ...
  return (
    <Layout>
      <Helmet>
        {/* Basic Meta Tags - Using database values with fallbacks */}
        <title>{blog.meta_title || `${blog.title} - RevivoHeal Blog`}</title>
        <meta name="description" content={blog.meta_description || blog.excerpt || `Read about ${blog.title} on RevivoHeal's blog about Hijama cupping therapy and natural healing.`} />
        {blog.meta_keywords && <meta name="keywords" content={blog.meta_keywords} />}
        <link rel="canonical" href={blog.canonical_url || currentUrl} />
        <meta name="robots" content={blog.robots_meta || "index, follow"} />
        <meta name="author" content={blog.author} />
        
        {/* Open Graph Meta Tags - Using database values with fallbacks */}
        <meta property="og:title" content={blog.og_title || blog.meta_title || blog.title} />
        <meta property="og:description" content={blog.og_description || blog.meta_description || blog.excerpt || `Read about ${blog.title} on RevivoHeal's blog.`} />
        <meta property="og:type" content={blog.og_type || 'article'} />
        <meta property="og:url" content={currentUrl} />
        <meta property="og:image" content={blog.og_image || validImageUrl || 'https://i.ibb.co/TxLzP5H7/revivo-heal-logo.png'} />
        <meta property="og:site_name" content="RevivoHeal" />
        
        {/* Twitter Card Meta Tags - Using database values with fallbacks */}
        ...
        <meta name="twitter:description" content={blog.twitter_description || blog.meta_description || blog.excerpt || `Read about ${blog.title} on RevivoHeal's blog.`} />
        <meta name="twitter:image" content={blog.twitter_image || blog.og_image || validImageUrl || 'https://i.ibb.co/TxLzP5H7/revivo-heal-logo.png'} />
        ...
        {/* Schema Markup - Using database value or generated fallback */}
            ...
            <div className="sticky top-24 z-20 space-y-4 sm:space-y-6">
              ...
              <Card className="bg-gradient-to-br from-brand-green to-brand-green/80 text-white shadow-lg w-full">
                <CardHeader className="pb-2 sm:pb-4">
                  <CardTitle className="text-lg sm:text-xl">Book Your Hijama Session</CardTitle>
                </CardHeader>
